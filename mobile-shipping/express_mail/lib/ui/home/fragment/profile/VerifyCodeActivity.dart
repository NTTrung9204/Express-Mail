...
import 'package:express_mail/ui/home/fragment/profile/ResetPasswordActivity.dart';
...
class VerifyCodeActivity extends StatefulWidget {
  ...
  const VerifyCodeActivity({super.key, required this.loginResponse});
...
class _VerifyCodeActivityState extends State<VerifyCodeActivity> {
  ...
  final ProfileViewModel _forgotViewModel = ProfileViewModel();
  ...
  Future<void> _sendOtp() async {
    bool success = await _forgotViewModel.reset(
      widget.loginResponse.user.email,
    );
    if (!mounted) return;

    if (!success) {
      if (_forgotViewModel.errorMessageReset != null) {
        _showErrorDialog(_forgotViewModel.errorMessageReset!);
      }
    }
  }
  ...
  Future<void> _onResend() async {
    ...
    try {
      await _forgotViewModel.reset(widget.loginResponse.user.email);
    } catch (_) {}
    if (!mounted) return;
  ...
  Future<void> _onVerify() async {
    ...
    try {
      bool success = await _forgotViewModel.confirm(
        widget.loginResponse.user.email,
        code,
      );
      ...
      if (success) {
        if (mounted) {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (_) => ResetPasswordActivity(
