...
const menuItems = [
  { label: "Trang chủ", icon: <DashboardIcon />, path: "/home" },
  { label: "Quản lý đơn hàng", icon: <Inventory2Icon />, path: "/order-draft" },
  { label: "Nhập từ Excel", icon: <DownloadIcon />, path: "/import-excel" },
  { label: "Quản lý kho", icon: <StoreIcon />, path: "/warehouse" },
  { label: "COD", icon: <AttachMoneyIcon />, path: "/cod" },
  { label: "Khiếu nại", icon: <NoteAltIcon />, path: "/complaints" },
  { label: "Quyền hạn", icon: <VpnKeyIcon />, path: "/permissions" },
    ...
    <aside className="bg-white w-64 min-h-screen border-r border-gray-200 flex flex-col p-3">
      <div className="flex flex-col items-center p-4 border border-gray-400 rounded-sm">
        ...
        <div className="flex items-center flex-col mt-2">
          ...
          <button className="bg-gray-200 w-full mt-2 p-2 rounded-sm hover:bg-orange-400 hover:text-white transition cursor-pointer">
            Xác thực tài khoản
          </button>
        </div>
      </div>
          ...
          {menuItems.map((item) => (
            <li key={item.label}>
              <NavLink
                to={item.path}
                className={({ isActive }) =>
                  `w-full flex items-center px-4 py-2 text-left transition font-medium cursor-pointer ${
                    isActive
